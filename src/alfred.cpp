...
bool send_json_streams(std::string scenario_id, std::string emo_uuid, std::string exec_uuid, bool del_files = true) {
    ...
    std::cout << "core_str: " << core.dump() << std::endl;    
...
bool send_files(std::string scenario_id, std::string emo_uuid, std::string exec_uuid) {
    ...
    std::string manuretransport_filename;
    if (exec_uuid.find("/") != std::string::npos) {
        land_filename = fmt::format("{}/output/nsga3/{}_impbmpsubmittedland.parquet", msu_cbpo_path, exec_uuid);
        animal_filename = fmt::format("{}/output/nsga3/{}_impbmpsubmittedanimal.parquet", msu_cbpo_path, exec_uuid);
        manuretransport_filename = fmt::format("{}/output/nsga3/{}_impbmpsubmittedmanuretransport.parquet", msu_cbpo_path, exec_uuid);
    }
    else{
        land_filename = fmt::format("{}/output/nsga3/{}/{}_impbmpsubmittedland.parquet", msu_cbpo_path, emo_uuid, exec_uuid);
        animal_filename = fmt::format("{}/output/nsga3/{}/{}_impbmpsubmittedanimal.parquet", msu_cbpo_path, emo_uuid, exec_uuid);
        manuretransport_filename = fmt::format("{}/output/nsga3/{}/{}_impbmpsubmittedmanuretransport.parquet", msu_cbpo_path, emo_uuid, exec_uuid);
    }
